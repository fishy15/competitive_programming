...
void solve() {
    ...
    const auto &dp = prev_dp;

    vector<int> cnt_by_popcnt(n+1);
    rep(i, 0, 1 << n) {
        cnt_by_popcnt[__builtin_popcount(i)] += dp[i];
    }

    mi ans = mi(m).pow(n);
    rep(i, 2, m+1) {
        rep(pc, 0, n+1) {
            auto arrange_big = mi(m - i + 1).pow(pc);
            auto arrange_small = mi(i - 1).pow(n - pc);
            ans += mi(cnt_by_popcnt[pc]) * arrange_big * arrange_small;
        }
    }
