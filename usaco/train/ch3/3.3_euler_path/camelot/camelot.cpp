...
void find_dist(int i) {
    ...
    queue<array<int, 4>> q;
    q.push(arr);
    q.push({arr[0], arr[1], 1, max(abs(knights[i].first - king.first), abs(knights[i].second - king.second))});
    while (!q.empty()) {
        ...
        for (auto m : moves) {
            ...
            if (new_x >= 0 && new_y >= 0 && new_x < r && new_y < c) {
                if (cur[2] == 0) {
                    ...
                    if (!dist[i][new_x][new_y][1]) {
                        q.push({new_x, new_y, 1, \
                                cur[3] + 1 + max(abs(new_x - king.first), abs(new_y - king.second))});
                    }
                } else {
                    if (!dist[i][new_x][new_y][1]) {
                        q.push({new_x, new_y, 1, cur[3] + 1});
                    }
                }
            }
        }
